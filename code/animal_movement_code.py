...
def animal_movement(properties, params, day):
    ...
    for premise_index in range(len(properties)):
        ...
        if not ((day - properties[premise_index].movement_start_day)%params['movement_frequency']):
            ...
            # if the property hasn't been culled AND it hasn't been vaccinated
            if not properties[premise_index].culled_status and not properties[premise_index].vaccination_status:

                # if there is movement
                prob_movement = np.random.rand()
                if prob_movement < params['movement_probability']:
                    
                    # where can the animals move to 
                    moving_to_premise_indices = []
                    for i in range(params['n']):
                        #property hasn't been culled and isn't the moving from property and isn't vaccinated 
                        if not properties[i].culled_status and i != premise_index and not properties[i].vaccination_status: 
